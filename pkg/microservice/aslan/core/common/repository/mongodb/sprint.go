...
func (c *SprintColl) BulkUpdateStages(ctx *handler.Context, objs []*models.Sprint) error {
	var ms []mongo.WriteModel
	for _, obj := range objs {
		ms = append(ms,
			mongo.NewUpdateOneModel().
				SetFilter(bson.D{{"_id", obj.ID}, {"is_archived", false}}).
	...
	if _, err := c.BulkWrite(mongotool.SessionContext(ctx, c.Session), ms); err != nil {
		return err
	}
	return nil
}
...
func (c *SprintColl) UpdateStages(ctx *handler.Context, idStr string, obj []*models.SprintStage) error {
	id, err := primitive.ObjectIDFromHex(idStr)
	if err != nil {
		return err
	}
