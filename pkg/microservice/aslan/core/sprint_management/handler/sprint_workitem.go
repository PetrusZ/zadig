...
// @Param 	sprint_update_time		query		int64									true	"sprint update time"
// @Param 	workitem_update_time	query		int64									true	"sprint workitem update time"
// @Success 200
...
func MoveSprintWorkItem(c *gin.Context) {
	...
	sprintUpdateTime, err := strconv.ParseInt(c.Query("sprint_update_time"), 10, 64)
	if err != nil {
		ctx.RespErr = e.ErrInvalidParam.AddDesc("sprint_update_time should be int64")
		return
	}
	workitemUpdateTime, err := strconv.ParseInt(c.Query("workitem_update_time"), 10, 64)
	if err != nil {
		ctx.RespErr = e.ErrInvalidParam.AddDesc("workitem_update_time should be int64")
		return
	}
	...
	ctx.RespErr = service.MoveSprintWorkItem(ctx, c.Param("id"), c.Query("stageID"), index, sprintUpdateTime, workitemUpdateTime)
}
...
// @Success 200
// @Router /api/aslan/sprint_management/v1/sprint_workitem/task/exec [post]
...
// @Router /api/aslan/sprint_management/v1/sprint_workitem/task [get]
func ListSprintWorkItemTask(c *gin.Context) {
	...
	if !ctx.Resources.IsSystemAdmin {
		...
		if !ctx.Resources.ProjectAuthInfo[projectName].IsProjectAdmin &&
			!ctx.Resources.ProjectAuthInfo[projectName].SprintManagement.View {
			ctx.UnAuthorized = true
			return
		}
	}
	...
	ctx.Resp, ctx.RespErr = service.ListSprintWorkItemTask(ctx, opt)
}
