...
// @Param 	update_time		query		int64									true	"sprint workitem update time"
// @Success 200
...
func MoveSprintWorkItem(c *gin.Context) {
	...
	updateTime, err := strconv.ParseInt(c.Query("update_time"), 10, 64)
	if err != nil {
		ctx.RespErr = e.ErrInvalidParam.AddDesc("update_time should be int64")
		return
	}
	...
	ctx.RespErr = service.MoveSprintWorkItem(ctx, c.Param("id"), c.Query("stageID"), index, updateTime)
}
...
func CloneSprintWorkItemTask(c *gin.Context) {
	...
	ctx.Resp, ctx.RespErr = service.CloneSprintWorkItemTask(ctx, c.Query("task_id"))
}
