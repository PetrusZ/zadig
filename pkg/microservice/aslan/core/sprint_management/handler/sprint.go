...
func CreateSprint(c *gin.Context) {
	...
	if !ctx.Resources.IsSystemAdmin {
		...
		if !ctx.Resources.ProjectAuthInfo[projectName].IsProjectAdmin &&
			!ctx.Resources.ProjectAuthInfo[projectName].SprintManagement.Create {
			ctx.UnAuthorized = true
			return
		}
	}
	...
	internalhandler.InsertDetailedOperationLog(c, ctx.UserName, projectName, setting.OperationSceneSprintManagement, "创建", "迭代管理-迭代", fmt.Sprintf("流程ID: %s, 迭代名称: %s", c.Query("templateID"), c.Query("name")), "", ctx.Logger, "")
...
func UpdateSprintName(c *gin.Context) {
	...
	if !ctx.Resources.IsSystemAdmin {
		...
		if !ctx.Resources.ProjectAuthInfo[projectName].IsProjectAdmin &&
			!ctx.Resources.ProjectAuthInfo[projectName].SprintManagement.Edit {
			ctx.UnAuthorized = true
			return
		}
	}
...
func DeleteSprint(c *gin.Context) {
	...
	if !ctx.Resources.IsSystemAdmin {
		...
		if !ctx.Resources.ProjectAuthInfo[projectName].IsProjectAdmin &&
			!ctx.Resources.ProjectAuthInfo[projectName].SprintManagement.Delete {
			ctx.UnAuthorized = true
			return
		}
	}
...
func ArchiveSprint(c *gin.Context) {
	...
	if !ctx.Resources.IsSystemAdmin {
		...
		if !ctx.Resources.ProjectAuthInfo[projectName].IsProjectAdmin &&
			!ctx.Resources.ProjectAuthInfo[projectName].SprintManagement.Archive {
			ctx.UnAuthorized = true
			return
		}
	}
