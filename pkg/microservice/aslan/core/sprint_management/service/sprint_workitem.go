...
func CreateSprintWorkItem(ctx *handler.Context, args *models.SprintWorkItem) error {
	...
	if !found {
		return fmt.Errorf("Stage %s not found", args.StageID)
	}

	err = mongodb.NewSprintCollWithSession(session).UpdateStages(ctx, args.SprintID, sprint.Stages)
	if err != nil {
		return errors.Wrapf(err, "Update sprint %s stages", args.SprintID)
	}
...
func GetSprintWorkItem(ctx *handler.Context, id string) (*SprintWorkItem, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Get activities by workitem id %s", id)
	}
...
func DeleteSprintWorkItem(ctx *handler.Context, id string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Get sprint by id %s", workitem.SprintID)
	}
	for _, stage := range sprint.Stages {
	...
	if err != nil {
		return errors.Wrap(err, "Delete sprint workitem")
	}
...
func UpdateSprintWorkItemTitle(ctx *handler.Context, id, title string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Get workitems by sprint id %s", id)
	}

	err = mongodb.NewSprintWorkItemCollWithSession(session).UpdateTitle(ctx, id, title)
	if err != nil {
		return errors.Wrapf(err, "Update sprint workitem %s title", id)
	}
...
func UpdateSprintWorkItemDescpition(ctx *handler.Context, id, description string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Update sprint workitem %s description", id)
	}

	return nil
}

func MoveSprintWorkItem(ctx *handler.Context, id, stageID string, index int, updateTime int64) error {
	...
	if workitem.UpdateTime > updateTime {
		return errors.New("Sprint workitem has been updated, please refresh and try again")
	}
	...
	if sprint.UpdateTime > updateTime {
		return errors.New("Sprint has been updated, please refresh and try again")
	}

	if workitem.StageID == stageID {
		for _, stage := range sprint.Stages {
			if index > len(stage.WorkItemIDs)+1 {
				return fmt.Errorf("Invalid stage index, requset %d but size is %d", index, len(stage.WorkItemIDs)+1)
			}
			if index == len(stage.WorkItemIDs)+1 {
...
func UpdateSprintWorkItemOwners(ctx *handler.Context, id, verb string, owners []types.UserBriefInfo) error {
	...
	default:
		return errors.New("Update sprint workitem owners invalid verb")
	}
	...
	if err != nil {
		return errors.Wrapf(err, "Update sprint workitem %s owners", id)
	}
...
func ExecSprintWorkItemWorkflow(ctx *handler.Context, workitemIDs []string, workflowName string, workflowArgs *commonmodels.WorkflowV4) error {
	...
	if err != nil {
		return errors.Wrapf(err, "List workitems by ids %v", workitemIDs)
	}
	...
	for _, id := range workitemIDs {
		if !foundWorkitemIDSet.Has(id) {
			return fmt.Errorf("Workitem %s not found", id)
		}
	}
	...
	if err != nil {
		return errors.Wrap(err, "Create workflow task")
	}
	...
	if err != nil {
		return errors.Wrap(err, "Create sprint workitem task")
	}

	return errors.Wrapf(err, "Create workflow task %s", workflowName)
}
...
func CloneSprintWorkItemTask(ctx *handler.Context, workitemTaskID string) (*CloneSprintWorkItemTaskResponse, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Get workitem task by id %v", workitemTaskID)
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Clone workflow task %s/%d", task.WorkflowName, task.WorkflowTaskID)
	}
...
func ListSprintWorkItemTask(ctx *handler.Context, req *ListSprintWorkItemTaskRequset) (*ListSprintWorkItemTaskResponse, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "Get workitem task by workflow name %v", req.WorkflowName)
	}
