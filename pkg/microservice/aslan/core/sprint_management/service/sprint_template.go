...
func CreateSprintTemplate(ctx *handler.Context, args *models.SprintTemplate) error {
	...
	err := args.Lint()
	if err != nil {
		return errors.Wrap(err, "Invalid sprint template")
	}
	...
	if err != nil {
		return errors.Wrap(err, "Create sprint template error")
	}
	return nil
}
...
func GetSprintTemplate(ctx *handler.Context, id string) (*models.SprintTemplate, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "GetSprintTemplate")
	}
	...
	if err != nil {
		return nil, errors.Wrapf(err, "ListWorkflow %s", workflowNames)
	}

	changed := false
	for _, workflow := range workflowList {
		if _, ok := workflowMap[workflow.Name]; !ok {
			return nil, errors.Errorf("Workflow %s not found in sprint template", workflow.Name)
		}
	...
	if changed {
		...
		if err != nil {
			return nil, errors.Wrapf(err, "Update sprint template %s", sprintTemplate.Name)
		}
	}
...
func DeleteSprintTemplate(ctx *handler.Context, username, id string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Get sprint template %s", id)
	}
...
func UpdateSprintTemplate(ctx *handler.Context, sprintTemplateID string, args *models.SprintTemplate) error {
	...
	if err != nil {
		return errors.Wrap(err, "Get sprint template")
	}

	if template.UpdateTime > args.UpdateTime {
		return errors.New("Sprint template has been updated by others, please refresh and try again")
	}
	...
	if err != nil {
		return errors.Wrapf(err, "List sprints by template id %s", sprintTemplateID)
	}
	...
	if len(sprints) > 0 {
		...
		if err != nil {
			return errors.Wrap(err, "Bulk Update Sprints Stages")
		}
	}

	if err = mongodb.NewSprintTemplateCollWithSession(session).Update(ctx, template); err != nil {
		return errors.Wrap(err, "Update sprint template")
	}
...
func ListSprintTemplate(ctx *handler.Context, opt *ListSprintTemplateOption) (*ListSprintTemplateResp, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "ListSprintTemplate")
	}
